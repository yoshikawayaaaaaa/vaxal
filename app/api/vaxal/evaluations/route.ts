...
export async function GET(request: NextRequest) {
  try {
    ...
    const engineerUserId = searchParams.get('engineerUserId')

    const where = engineerUserId
      ? { engineerUserId: parseInt(engineerUserId) }
      : {}

    const evaluations = await prisma.engineerEvaluation.findMany({
      where,
      include: {
        engineerUser: {
          select: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const evaluation = await prisma.engineerEvaluation.create({
      data: {
        engineerUserId: parseInt(body.engineerUserId),
        evaluatedByVaxalId: parseInt(session.user.id),
