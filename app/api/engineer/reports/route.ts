...
import { parseJSTToUTCDebug } from '@/lib/date-utils-debug'
      ...
      if (imageCount > 0) {
        ...
        const report = await prisma.report.create({
          data: {
            ...
            remainingWorkDate: reportType === 'COMPLETION' && engineerInfo.remainingWorkDate ? (() => {
              const debugResult = parseJSTToUTCDebug(engineerInfo.remainingWorkDate)
              console.log('本番環境デバッグ:', debugResult)
              return parseJSTToUTC(engineerInfo.remainingWorkDate)
            })() : null,
